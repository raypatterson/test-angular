...
require "./lib/extensions/compass-configurator"
...
puts "bower_dir: #{bower_dir}"
puts "watch_dir: #{watch_dir}"
...
helpers do
  def get_title_copy( count = 1, random = false, punctuate = false )
    if random == true
      count = rand count
    end
    s = "#{lorem.word} "
  ...
  def get_body_copy( count = 1, random = false )
    if random == true
      count = rand count
    end
    s = "#{lorem.paragraph}. "
  ...
  def get_image( size = '400x400', text = 'FPO', background_color = nil)
    if background_color == nil
      return lorem.image( size, :random_color => true, :text => text )
    else
      return lorem.image( size, :background_color => background_color, :color => 'fff', :text => text )
    end
  end
end
...
activate :compass_configurator do | configurator |
  if $env == Cfg.get_localhost_env
    configurator.output_style = :expanded
    configurator.line_comments = true
  else
    configurator.output_style = :compact
    configurator.line_comments = false
  end
end
